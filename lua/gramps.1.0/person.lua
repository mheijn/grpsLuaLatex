...
function Pers.citation(handle)
	local p=Pers(handle)
	if not p.citation then
		p.citation =citation.of_object(p.handle)
	end 
	return p.citation
end
-------------------------------------
...
if arg ~= nil and arg[0] == string.sub(debug.getinfo(1,'S').source,2) then
    ...
    --pass
    print(Pers.fullname('I0692'))
    print(Pers.fullname('I999009'))
 --   local p = Pers('I0692')
...
--	pers.Residence(p1)
	...
	print(Pers.fullname('I8786'))
	for i,c in pairs(Pers.citation('I8786')) do
		print(i,c,citation[c].gramps_id)
	end
else
    return(Pers)
end
