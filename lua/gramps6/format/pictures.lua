...
function Picture.image(h,embed,with_link)
	...
	if textwidth > 120 then 
        longsize = 0.45*textwidth else  longsize = 0.7*textwidth end
	...
	if tonumber(x)>tonumber(y) then y=longsize*y/x; x=longsize
...
function Picture_xml.image(h,embed)
    embed = (embed==nil) or embed
	...
	local s=""
	if embed then 
        s ='\\begin{Figure}\\begin{center}\n'
    end
    s=s..caption.."\\\\ \n"
    s=s..'\\includegraphics[width='..x..'pt,height='..y..'pt]{'..filename.."}\\\\ \n"
    s=s..legenda(media).."\\\\ \n" 
