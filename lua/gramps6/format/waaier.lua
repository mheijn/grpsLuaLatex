local wi = {}
local grps = gramps6 or require("gramps6")
_ = _ or require("mh.gettext")

function gramps6.Event:treedate()
    s=""
...
function wi.set_voorouder_trees(voorouders,generations,tree_size)
    ...
    for g=1,generations,tree_size-1 do
        ...
        if voorouders[g] then for k= 2^(g-1),2^(g)-1 do
            ...
            if h then
                trees[#trees][1]=k -- tree number
                trees[#trees][2]=h -- handle
                k = (gramps6.Person(h):hasParents()>0) and k or 0
                if notfirst then
                    trees[#trees-1][4]=k --handle left
                else
                    trees[#trees][3]=0
                end
                notfirst=true
                table.insert(trees,{0,0,k,0}) --handle right
            end
        end end
...
local function ancestor_tree_data(h,maxgen,keku,gen,kekule,links)
    ...
    if gen==1 then if p:hasChildren()  then table.insert(links,{gramps6.Person(h).gramps_id,math.floor(keku/(2^(maxgen-1)))})
...
function wi.tree(ph,depth,treebase,keku,left_id,right_id)
    ...
    for i,l in ipairs(links) do
        if i==1 then if 0<l[2] then
            ...
            if 0 < gramps6.Person(l[1]):hasParents() then
                print(string.format("\\node [] (c%d) at (%s%s.north) {\\hyperlink{TREE%d%s}{$\\uparrow$}};",
                    i,p.gramps_id,l[1],l[2],treebase))
            end
        end
    end
...
if arg ~= nil and arg[0] == string.sub(debug.getinfo(1,'S').source,2) then
	...
	require("gramps")
   	local grps_util = require("gramps6.format.util")
    ...
    --local p=gramps6.Person('I1119')
   	local voorouders = grps_util.ancestors('I14180',15)

    local trees = wi.set_voorouder_trees(voorouders,15,4)
    for j,v in pairs(trees) do
        print (table.tostring(v))
    end
    --wi.tree('I1119',3,gramps6.Person('I1119').gramps_id,1)
else
    return wi
end
