...
if gramps6 and not gramps6.Date then
    gramps6.Date      = {}
    gramps6.Date.__file = debug.getinfo(1, "S").source:sub(2)
  ...
  function gramps6.Date.MakeDate(d,m,y,l,weekday)
    local s =""
    if weekday and d>0 and m>0 and y>0 then s=s.. days[gramps6.Date.weekday(d,m,y)].." " end
    if l=="nl"then
  ...
  function gramps6.Date.MDate(d,m,y,l)
    local s =""
    if l=="nl"then
      if d>0 then s=s..tostring(d) end
  ...
  function gramps6.Date.long(date,weekday)
    --for k,v in pairs(self) do print(k,v) end
    local s=""

    local d =date.dateval
    if type(d) ~= "table" then return "" end
    lan = locality.language()
    if date.modifier < gramps6.Date.Modifier.RANGE then
      s = s .. modifier[date.modifier+1] .. quality[date.quality+1]..gramps6.Date.MakeDate(d[1],d[2],d[3],lan,weekday)
    elseif date.modifier < gramps6.Date.Modifier.TEXTONLY then
      s = s .. modifier[date.modifier+1].. quality[date.quality+1] ..
          gramps6.Date.MakeDate(_DnotEq(d[1],d[5]),_DnotEq(d[2],d[6]),_DnotEq(d[3],d[7]),lan,weekday)..
          --gramps6.Date.MakeDate(d[1],d[2],d[3],lan,weekday)..
          _(" and ") ..gramps6.Date.MakeDate(d[5],d[6],d[7],lan,weekday)
    else
      s = s ..  self.text
    end
    return s-- .." ("..lan..")"
  ...
  function gramps6.Date:short()
    ...
    lan = locality.language()
    local s=""
    if self.modifier < gramps6.Date.Modifier.RANGE then
      s = s .. short_modifier[self.modifier+1].. short_quality[self.quality+1] ..gramps6.Date.MDate(d[1],d[2],d[3],lan)
    elseif self.modifier < gramps6.Date.Modifier.TEXTONLY then
      s = s .. short_quality[self.quality+1].. short_modifier[self.modifier+1]..
            gramps6.Date.MDate(_DnotEq(d[1],d[5]),_DnotEq(d[2],d[6]),_DnotEq(d[3],d[7]),lan)..
            " - "..gramps6.Date.MDate(d[5],d[6],d[7],lan)
    end
    return s
  end
  ...
  function gramps6.Date.weekday(day, month, year)
  ...
  function gramps6.Date.realdate(date)
      local d =date.dateval
      if d then
          return (date.modifier == gramps6.Date.Modifier.NONE and d[1]>0 and d[2]>0 and d[3]>0)
  ...
  -- @param d @{Gramps_date}
  -- @return boolean (true if real)
  function gramps6.Date:aboutdate()
      local d =self.dateval
      
      return d and ((d[2]==gramps6.Date.Modifier.NONE or d[2]==gramps6.Date.Modifier.ABOUT)  and d[4][3]>0)
  end
  ...
  function gramps6.Date.from_days_to_years(days)
  ...
  function gramps6.Date:days_from_year_0()-- Moet veranderd
      local d =self.dateval
  ...
  function gramps6.Date.age(start,stop)
    local sa=start.dateval
    local ss=stop.dateval
    
    if sa and ss then
      return (gramps6.Date.realdate(start) and gramps6.Date.realdate(stop)),
          substract(tonumber(ss[1]), tonumber(ss[2]),tonumber(ss[3]),tonumber(sa[1]),tonumber(sa[2]),tonumber(sa[3]))
    end
    return  nil
  end
  ---
...
if arg ~= nil and arg[0] == string.sub(debug.getinfo(1,'S').source,2) then
    ...
    util.printfun(gramps6.Date,"Ndate")
    local da1=gramps6.Date({0,0,0,{1,2,2025,0,2,3,2025},"datum 1",0,0})
    local da2=gramps6.Date({0,0,0,{1,2,2023,0,2,3,2023},"datum 2",0,0})
    util.printfun(da2,"datum d2")
    local da3=gramps6.Date({0,0,0,{1,2,2023,0,2,3,1962},"datum 3",0,0})
 ...
 --   da1:long()
    ...
    print(gramps6.Date.from_days_to_years(738920))
